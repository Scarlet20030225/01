...
Dx12Application::Dx12Application(unsigned int width, unsigned int height, std::wstring title)
	: title_(title)
	, windowWidth_(width)
...
// 初期化処理
void Dx12Application::OnInit(HWND hwnd)
{
	LoadPipeline(hwnd);
	LoadAssets();
}

void Dx12Application::LoadPipeline(HWND hwnd)
{
...
// D3D12Deviceの生成
void Dx12Application::CreateD3D12Device(IDXGIFactory6* dxgiFactory, ID3D12Device** d3d12device)
{
...
void Dx12Application::ThrowIfFailed(HRESULT hr)
{
	if (FAILED(hr))
	{
		// hrのエラー内容をthrowする
